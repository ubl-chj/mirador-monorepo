 ...
 * @private
 */
export const WorkspaceAdd = (props) => {
  ...
  return (
    <div className={ns('workspace-add')}>
      ...
      <Fab variant="extended" disabled={addResourcesOpen} className={classes.fab} color="secondary" onClick={() => setAddResourcesVisibility(true)}>
        <AddIcon />
        {t('addResource')}
      </Fab>
      ...
      >
        ...
        >
          <AppBar position="absolute" color="secondary" onClick={() => setAddResourcesVisibility(false)}>
            <Toolbar>
          ...
          <ManifestForm onCancel={() => setAddResourcesVisibility(false)} />
        </Paper>
      </Drawer>
    </div>
  );
